                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMCLB ought to be enough for anybody." - Bill Gates, 2017
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzMCLB->setToolTip(tr("zMCLB is currently disabled due to maintenance."));

        ui->pushButtonSpendzMCLB->setEnabled(false);
        ui->pushButtonSpendzMCLB->setToolTip(tr("zMCLB is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_pushButtonMintzMCLB_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMCLB is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMCLB...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMCLB in ") +
...
void PrivacyDialog::on_pushButtonSpendzMCLB_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMCLB is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMCLB
        sendzMCLB();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMCLB
    sendzMCLB();
}
...
void PrivacyDialog::sendzMCLB()
{
    ...
    // Warn for additional fees if amount is not an integer and change as zMCLB is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MCLB </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMCLB</b>";
    ...
    // use mints from zMCLB selector if applicable
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMCLB Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMCLB </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMCLB "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMCLB "));
